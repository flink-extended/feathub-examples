...
with extra features and sink the output to s3 for offline training. It involves 
the following steps:
...
4. Output the batch of purchase events backfilled with the extra features to S3.

5. Re-read the purchase events from s3 and output them to a local file.
...
Please execute the following commands under the `flink-read-write-s3`
...
2. Start the Flink cluster and object store.
